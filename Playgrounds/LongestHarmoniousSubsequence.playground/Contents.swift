...
func findLHS(_ nums: [Int]) -> Int {
    ...
    var numsByValue = [Int: Int]()
    for num in nums {
        numsByValue[num, default: 0] += 1
    }

    var result = 0

    for num in numsByValue.keys {
        if let greater = numsByValue[num + 1] {
            result = max(numsByValue[num, default: 0] + greater, result)
        }
    }
